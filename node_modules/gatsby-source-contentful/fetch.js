  ...
  try {
    ...
    if (e.code === `ENOTFOUND`) {
      ...
      if (e.responseData.status === 404 && contentfulClientOptions.environment && contentfulClientOptions.environment !== `master`) {
        ...
        details = `Unable to access your space. Check if ${chalk.yellow(`environment`)} is correct and your ${chalk.yellow(`accessToken`)} has access to the ${chalk.yellow(contentfulClientOptions.environment)} and the ${chalk.yellow(`master`)} environments.`;
        errors = {
...
${formatPluginOptionsForCLI(pluginConfig.getOriginalPluginOptions(), errors)}`
  ...
  try {
    syncProgress = reporter.createProgress(`Contentful: ${syncToken ? `Sync changed items` : `Sync all items`}`, currentPageLimit, 0);
    syncProgress.start();
