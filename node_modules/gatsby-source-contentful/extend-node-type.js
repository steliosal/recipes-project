...
var _ref, _process$env$GATSBY_C;
...
} = require(`./schemes`); // By default store the images in `.cache` but allow the user to override
...
if (process.env.GATSBY_REMOTE_CACHE) {
  console.warn(`Note: \`GATSBY_REMOTE_CACHE\` will be removed soon because it has been renamed to \`GATSBY_CONTENTFUL_EXPERIMENTAL_REMOTE_CACHE\``);
}

if (process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_REMOTE_CACHE) {
  console.warn(`Please be aware that the \`GATSBY_CONTENTFUL_EXPERIMENTAL_REMOTE_CACHE\` env flag is not officially supported and could be removed at any time`);
}

const REMOTE_CACHE_FOLDER = (_ref = (_process$env$GATSBY_C = process.env.GATSBY_CONTENTFUL_EXPERIMENTAL_REMOTE_CACHE) !== null && _process$env$GATSBY_C !== void 0 ? _process$env$GATSBY_C : process.env.GATSBY_REMOTE_CACHE) !== null && _ref !== void 0 ? _ref : path.join(process.cwd(), `.cache/remote_cache`);
const CACHE_IMG_FOLDER = path.join(REMOTE_CACHE_FOLDER, `images`); // Promises that rejected should stay in this map. Otherwise remove promise and put their data in resolvedBase64Cache
...
const isImage = image => {
  ...
  return [`image/jpeg`, `image/jpg`, `image/png`, `image/webp`, `image/gif`].includes(image === null || image === void 0 ? void 0 : (_image$file = image.file) === null || _image$file === void 0 ? void 0 : _image$file.contentType);
...
const getBase64Image = imageProps => {
  ...
  const requestUrl = `https:${imageProps.baseUrl}?w=20`; // Prefer to return data sync if we already have it
  ...
  const urlSha = shasum.digest(`hex`); // TODO: Find the best place for this step. This is definitely not it.

  fs.mkdirSync(CACHE_IMG_FOLDER, {
    recursive: true
  });
  ...
  const loadImage = async () => {
    ...
    try {
      ...
      console.error(`Contentful:getBase64Image: failed to write ${body.length} bytes remotely fetched from \`${requestUrl}\` to: \`${cacheFile}\`\nError: ${e}`);
      throw e;
    }
  };
...
const resolveFixed = (image, options) => {
  ...
  const sortedSizes = sortBy(filteredSizes); // Create the srcSet.
...
exports.extendNodeType = ({
  ...
  const getDominantColor = async ({
    ...
    try {
      ...
      console.error(`[gatsby-source-contentful] Please install gatsby-plugin-sharp`);
      return `rgba(0,0,0,0.5)`;
    }
  };
